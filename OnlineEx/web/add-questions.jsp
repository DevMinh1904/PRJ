            ...
            <c:if test="${not empty questions}">
                ...
                <div style="margin-top: 15px;">
                    <c:forEach var="question" items="${questions}" varStatus="status">
                        <c:set var="correctOption" value="${question.correctOption}" />
                        <div style="border: 1px solid #ddd; padding: 10px; margin-bottom: 10px;">
                            <h4>Question ${status.index + 1}: ${question.questionText}</h4>
                          <p><strong>A:</strong> ${question.optionA} ${question.correctOption == 1 ? '(Correct)' : ''}</p>
<p><strong>B:</strong> ${question.optionB} ${question.correctOption == 2 ? '(Correct)' : ''}</p>
<p><strong>C:</strong> ${question.optionC} ${question.correctOption == 3 ? '(Correct)' : ''}</p>
<p><strong>D:</strong> ${question.optionD} ${question.correctOption == 4 ? '(Correct)' : ''}</p>
                        </div>
                    </c:forEach>
                </div>
            </c:if>
